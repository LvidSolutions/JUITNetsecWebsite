...
export async function consumeContactRateLimit({
  ...
  const safePrefix = String(keyPrefix).toLowerCase().replace(/[^a-z0-9:_-]/gu, '-').slice(0, 80) || 'juit:contact';
  const keys = [
    `${safePrefix}:ip:${windowId}:${addressHash}`,
    `${safePrefix}:email:${windowId}:${emailHash}`,
  ];
  ...
  try {
    response = await fetchImpl(`${upstashUrl}/multi-exec`, {
      ...
      body: JSON.stringify([
        ['INCR', keys[0]],
        ['EXPIRE', keys[0], windowSeconds * 2],
        ['INCR', keys[1]],
        ['EXPIRE', keys[1], windowSeconds * 2],
      ]),
  ...
  if (
    ...
    [0, 2].some((index) => result[index]?.error || !Number.isFinite(Number(result[index]?.result)))
  ) {
    throw new HttpError(503, TEMPORARY_UNAVAILABLE);
  }

  const ipCount = Number(result[0].result);
  const emailCount = Number(result[2].result);
  const retryAfter = Math.max(1, windowSeconds - (Math.floor(now / 1000) % windowSeconds));
  const remaining = Math.max(0, Math.min(max - ipCount, emailMax - emailCount));

  return {
    allowed: ipCount <= max && emailCount <= emailMax,
    remaining,
    retryAfter,
  };
}
